...
const Sites = ({ site, deleteProduct, type }: ISiteMap) => {
  ...
  const handleDelete = (id: string) => {
    axios({
        ...
        .then((res) => {
          dispatch(type === "retailer" ? deleteWebsite(id) : deleteResellWebsite(id));
  ...
  useLayoutEffect(() => {
    const degreeRotation = Math.floor((Math.random() *  (20 - 10)) + 10) * (Math.random() > 0.5 ? 1 : -1 ); 
  ...
  return (
    ...
    >
      {deleteProduct && (
        <span className="absolute -top-2 -left-2 bg-slate-300 rounded-full text-slate-600 p-0.5 opacity-90" onClick={() => handleDelete(site._id)}>
          <IoMdClose />
        </span>
      )}
      {site.name}
